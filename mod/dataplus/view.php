...
function dataplus_amend_record(){    
    ...
    if(!$mform->is_cancelled() && $form = $mform->get_data()){
        ...
        //itterate through all the columns included in the form and add data to the results array for use 
        // in generating update SQL
...
function dataplus_view_page_setup($js = null, $js_init = null, $css = null){
	dataplus_page_setup('/mod/dataplus/view.php',dataplus_get_querystring_vars(),(empty($dataplus->viewtablabel)) ? get_string('view','dataplus') : $dataplus->viewtablabel, $js, $js_init, $css);
...
if(($mode == 'delete' || $mode == 'deletesubmit') && empty($group)){
    if(has_capability('mod/dataplus:databaseedit', $context) || (($groupmode == 0 || groups_is_member($currentgroup)) && (has_capability('mod/dataplus:dataeditown', $context) || has_capability('mod/dataplus:dataeditothers', $context)))){           
