...
function dataplus_update_capabilities($dataplus, $roles = null){
    ...
    if(is_null($roles)){
        if (class_exists('ouflags')) {
            $roles = array('fullstudent'=>'student',
...
function dataplus_page_setup($filepath, $querystring, $page_title, $js=null, $js_init=null, $css=null) {
    ...
    //if the database isn't used, and we're not in setup mode, show the options to set up tables or import an existing dataplus database.
...
function dataplus_print_template_output($template, $results, $clear_actions = false){
    ...
    foreach($results as $result){
        ...
        foreach(dataplus_detail_supporting_actions() as $action){
            $record_template = dataplus_create_record_icon($action->name,$action->label,$record_template,$result->id,$result->creator_id,$result->group_id,$final_clear_actions);
        }
        ...
        foreach($result as $name=>$value){
            ...
            //as well as checking $form_field_type is set, this has the effect of stoping fields being printed if not in the correct group. 
            if(empty($type)){
 ...
 * @param int $rid record for which all records should be displayed (not necessary if only displaying a single comment)
 * @param int $ignore_end - only comments with an id below this will be displayed
 * @param int $ignore_start - only comments with id above this will be displayed
 * @param int $single - only a comment with this id will be displayed
 * @param boolean $return_html - return as HTML or display using Smarty.
 */
