...
class dataplus_file_helper {
    ...
    private function get_file_path($fileinfo){
    	...
    	return $CFG->wwwroot."/pluginfile.php/".$fileinfo['contextid']."/".$fileinfo['component']."/".$fileinfo['filearea'].$fileinfo['filepath'].$fileinfo['itemid']."/".$filename;
    }
    ...
    //rerturn the path for a top level file
     ...
     *
     * @param param string $type = 'image' or 'file'.  If null, returns the module instance dir
     */ 
     ...
     *
     * @param string $destination, if 'image' or 'file', only copy the image or file subdirs
     * @param array $exclude = array of filenames to exclude from a copy
     */	
    ...
    /**
     * delete all the files referenced in a database column for files or images (used when such a column is deleted or the type is changed)
     *
     * @param string $col_name, name of the column containing the file references
