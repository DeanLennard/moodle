...
// $Id: check_db_syntax.php,v 1.12 2010/06/09 13:46:46 stronk7 Exp $
...
/// Define some known false positives to take them out from errors report (nested array of => file
/// => regular expressions considered false positives)
    $fp = array (
          'install.php' => array(
                  /// Allow $distro->dbtype stuff to work in install
                  'empty\(\$distro-\>dbtype\)',
                  /// Allow $config->dbtype stuff to work in install
                  '= trim\(\$_POST\[\'dbtype\'\]',
                  /// Allow $config->dbtype stuff to work in install
                  'get_driver_instance\(\$config-|>dbtype'
              ),
          'admin/blocks.php' => array(
                  /// Tables can be dropped from blocks admin interface
                  'drop_plugin_tables.*\/blocks'
              ),
          'admin/health.php' => array(
                  /// health script is allowed to use $CFG->prefix when building suggested SQLs to
                  /// be shown
                  '\. \$CFG-\>prefix \.'
              ),
          'admin/modules.php' => array(
                  /// Tables can be dropped from modules admin interface
                  'drop_plugin_tables.*\/mod'
              ),
          'admin/qtypes.php' => array(
                  /// Tables can be dropped from qtype admin interface
                  'drop_plugin_tables.*\$QTYPES\[\$delete\]-\>'
              ),
          'lib/form/recaptcha.php' => array(
                  /// recaptcha form has own setAttributes method
                  'this->setAttributes'
              )
          );

/// List of exceptions that aren't errors (function declarations, comments, adodb usage from adodb
/// drivers and harcoded strings). Non reportable false positives
    ...
    if ($handle) {
        ...
        while (!feof($handle)) {
            ...
            foreach ($all_megarules as $name => $megarule) {
                if (!empty($megarule) && preg_match($megarule, $buffer)
                    && !preg_match($excludes, $buffer)) {
                    /// Let's see if that's a well known false positive
                    /// (only if executed from Moodle root)
                    if ($is_moodle_root && is_known_false_positive($fp, $file, $buffer,
                        $is_moodle_root)) {
                        /// Known false positive found, annotate it
                        if (!isset($falsepositives[$file])) {
                            $falsepositives[$file] = array();
                        }
                        $falsepositives[$file][] = "- NOTICE ( $name ) - line $line : ".
                            trim($buffer);
                        $countfalsepositives++;
                        break;
                    } else {
                        ...
                        echo "          - ERROR ( $name ) - line $line : ".trim($buffer).
                            LINEFEED;
                        $counterrors++;
                        break;
                    }
...
foreach ($errors as $file => $errarr) {
    ...
    echo LINEFEED . LINEFEED;
    echo "  Known false positive: ".count($falsepositives).
        " files with $countfalsepositives ocurrences".LINEFEED;
    echo "  (you should ignore these, although reviewing them from time to time isn't a bad idea
        either)".LINEFEED;
...
/// INTERNAL FUNCIONS
    ...
    /**
     * Given an array of search patterns, create one "megarule", with the specified 
