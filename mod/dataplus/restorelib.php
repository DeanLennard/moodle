...
function dataplus_restore_mods($mod, $restore) {
    ...
    if ($dataplus) {
        ...
        if ($restore->course_startdateoffset) {
            restore_log_date_changes('Database', $restore, $info['MOD']['#'],
                array('TIMEAVAILABLEFROM', 'TIMEAVAILABLETO', 'TIMEVIEWFROM', 'TIMEVIEWTO'));
        }

        $database->course = $restore->course_id;
        $database->name = backup_todb($info['MOD']['#']['NAME']['0']['#']);
        $database->intro = backup_todb($info['MOD']['#']['INTRO']['0']['#']);
        $database->timeavailablefrom = backup_todb(
            $info['MOD']['#']['TIMEAVAILABLEFROM']['0']['#']
        );
        $database->timeavailableto = backup_todb(
            $info['MOD']['#']['TIMEAVAILABLETO']['0']['#']
        );
        ...
        $database->requiredentriestoview = backup_todb(
            $info['MOD']['#']['REQUIREDENTRIESTOVIEW']['0']['#']
        );
        $database->maxentries = backup_todb($info['MOD']['#']['MAXENTRIES']['0']['#']);
        $database->maxentriesperuser = backup_todb(
            $info['MOD']['#']['MAXENTRIESPERUSER']['0']['#']
        );
        $database->studentediting = backup_todb($info['MOD']['#']['STUDENTEDITING']['0']['#']);
        $database->studenteditingother = backup_todb(
            $info['MOD']['#']['STUDENTEDITINGOTHER']['0']['#']
        );
        $database->teacherediting = backup_todb($info['MOD']['#']['TEACHEREDITING']['0']['#']);
        $database->teachereditingother = backup_todb(
            $info['MOD']['#']['TEACHEREDITINGOTHER']['0']['#']
        );
        $database->viewtablabel = backup_todb($info['MOD']['#']['VIEWTABLABEL']['0']['#']);
        $database->viewtabvisible = backup_todb($info['MOD']['#']['VIEWTABVISIBLE']['0']['#']);
        $database->singlerecordtablabel = backup_todb(
            $info['MOD']['#']['SINGLERECORDTABLABEL']['0']['#']
        );
        $database->singlerecordtabvisible = backup_todb(
            $info['MOD']['#']['SINGLERECORDTABVISIBLE']['0']['#']
        );
        ...
        $database->addrecordtablabel = backup_todb(
            $info['MOD']['#']['ADDRECORDTABLABEL']['0']['#']
        );
        $database->addrecordtabvisible = backup_todb(
            $info['MOD']['#']['ADDRECORDTABVISIBLE']['0']['#']
        );
        ...
        if (!defined('RESTORE_SILENTLY')) {
            echo "<li>".get_string("modulename", "dataplus")." \"".
                format_string(stripslashes($database->name), true)."\"</li>";
        }
...
function dataplus_restore_files($newid, $oldid, $restore) {
    ...
    $backup_path = $CFG->dataroot."/temp/backup/".$restore->backup_unique_code.
        "/moddata/dataplus/".$oldid;
